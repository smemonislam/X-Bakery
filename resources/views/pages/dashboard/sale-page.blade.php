...
@section('content')
    ...
    <script>
        ...
        function CalculateGrandTotal() {
            ...
            let Payable = 0;
            let Discount = 0;
            let discountPercenttage = (parseFloat(document.getElementById('discountP').value))

            InvoiceItemList.forEach((item, index) => {
                Total = Total + parseFloat(item['sale_price'])
            })

            if (discountPercenttage === 0){
                Vat = ((Total*5)/100).toFixed(2);
            }
            else{
                Discount = ((Total*discountPercenttage)/100).toFixed(2);
                Total = (Total-((Total*discountPercenttage)/100)).toFixed(2);
                Vat = ((Total*5)/100).toFixed(2);
            }
